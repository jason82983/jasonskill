...
批准清单是唯一授权范围。旧 Campaign 只查询和提示并行风险，不 Pause、Archive、Delete 或修改。Own ASIN、Mapped_SKUs[]/Advertised_SKUs[] 必须来自映射表、产品档案和 SellerSpace 三方一致验证；Benchmark/Product Target 只能作为研究或 Target 候选。

## 幂等与恢复

执行前检查 Campaign identity、名称 Sequence、Store、Marketplace、Own ASIN、Mapped_SKUs[]/Advertised_SKUs[]；生成唯一 `execution_id`。部分成功先 Read-Back，Recovery Plan 只覆盖失败对象，不能整批重跑。
