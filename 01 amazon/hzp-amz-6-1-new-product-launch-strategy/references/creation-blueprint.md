...
《6-1 新品广告创建决策》必须显示 Product Code、Product Name、Marketplace、Store、Own ASIN、SKU、Launch Goal、Launch Mode、Launch Window、Recommended Total Launch Budget、Phase 1 Released Budget、Initial Daily Budget、Maximum Daily Budget After Validation、Planned Campaign Count、Initial Keyword Count、Product Target ASIN Count 和 AI Recommendation。
...
每个 Ad Group 展示名称、Default Bid、Status；Advertised Product 展示 Own ASIN、SKU，并固定标记 `[Advertised Product｜Own Product]`。
...
Creation Checklist 必须覆盖 Identity、Own ASIN、SKU、Marketplace、Store、Campaign Names/Budgets、Ad Groups、Advertised Product、Keywords、Match Types、Product Targets、Target-Level Bids、Bidding Strategies、Placements、Auto Targeting、Negatives、Launch/Phase Budget、Validation Rules 和 6-2 Handoff。关键字段缺失时只能标记 `[Ready with Known Limitations]` 或 `[Not Ready to Create]`。
...
创建后用同一字段结构 Read-Back，核对 Campaign ID、名称、预算、策略、广告位、Ad Group、Own ASIN/SKU、Keyword/Match/Bid、Product Target、Auto、Negative 和 Status。
## Portfolio 字段（新增必填）
...
身份字段还必须记录 `Product_NewCode`、正式 `Product_Code`、Store 前缀、`Var_Code`、Advertised Child ASIN 和 Child SKU。变体映射不完整时只生成不可执行方案，禁止猜测子 ASIN/SKU；Campaign 名称使用共享 `format_campaign_name()`。
