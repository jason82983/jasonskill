...
- 6-0-2 的精准分类由 6-0-2 完成。6-1 不重新判断 `PRECISION/NOT_PRECISION`；`Precision` 不等于 Launch Priority、COR、Exact 或高 Bid。
...
历史 V2 方案中的 Campaign 命名描述已由现行 6-0-5 控制方式合同取代。当前名称使用 `{Product}.{Variant}.{AdType}-{Role}-{TargetType}-[IntentCode]-{Sequence}`：独立含Intent Code，共享不含Intent Code，不投不创建；历史 V2 命名只作迁移前参考，不自动改名。报告必须逐 Campaign 说明 Campaign Name、Product.Variant、AdType、Role、Tool、Intent/Purpose、Targets、Match、Bid、Placement、Bidding Strategy、Budget、Evidence、Validation Goal、Promotion/Exit Condition。
