...
Runtime Campaign Scope is mandatory: `ProductCode` + opaque `CampaignTag` → `CampaignPrefix=ProductCode.CampaignTag.`. The Campaign naming contract is `{ProductCode}.{CampaignTag}.{AdType}-{Role}-{TargetType}-[IntentCode]-{Seq}`. CampaignTag is not a Variant; verified real Variant remains a separate identity field. Independent requires exactly its 605 Intent Code; Shared has no Intent Code; No-invest creates no Campaign. Example `B2.M.SP-COR-EXA-SBG-01` is in scope for `B2` + `M` regardless of the actual product Variant. Campaign Name and run metadata must carry the exact scope.
