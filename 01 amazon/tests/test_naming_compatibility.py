...
def test_cases_a_to_l():
    ...
    # The new control-aware shared form uses the same base spelling without an Intent Code.
    shared = resolver.parse_campaign_name("B8.M.SP-COR-EXA-01")
    assert shared["status"] == "CURRENT_STANDARD" and shared["intent_code"] is None
    assert resolver.classify_campaign_name("B8.M.SP-COR-EXA-01", identity)["status"] == "CURRENT_STANDARD"
    assert resolver.classify_campaign_name("B8.M.SP-COR-EXA-01", {"formal_product_code": "B8", "var_code": "S"})["status"] == "NAME_IDENTITY_CONFLICT"
