...
6-2 不使用 `IsExact`（当前业务定义为“暂无用”）判断 ERP 精准词。关键词身份和经营异常只保留必要证据；精准词识别统一路由 6-0-1，精准泛词提取统一路由 `6-0-2｜精准泛词提取`（`hzp-amz-6-0-2-precision-broad-extraction`）。6-2 不复制聚类、精准泛词或 Rankability 算法。连接失败、无匹配行、ProId 缺失或冲突时保留错误状态并继续其他监控章节。
### Provider Boundary
...
6-2 只识别 ASIN 经营层面的关键词异常或机会（例如 Search Term 覆盖变化、核心词自然排名信号、广告/自然词转化差异），不再复制 ERP 精准词筛选、精准泛词生成、Search Intent 聚类或 Rankability 算法。精准词问题输出 `Route → 6-0-1`；精准泛词问题输出 `Route → 6-0-2｜精准泛词提取`（`hzp-amz-6-0-2-precision-broad-extraction`），并读取对应最新有效资产。
