...
- 6-2：只识别 ASIN 经营层面的关键词异常或机会，并路由 6-0-1；不复制 ERP 精准词筛选、聚类、精准泛词或 Rankability 算法。
...
6-2 可以读取共享 ERP Adapter 返回的关键词证据，识别 Search Term 覆盖、核心词排名信号、广告/自然转化差异等 ASIN 经营异常或机会。需要 ERP 精准词识别时输出 `Route → 6-0-1`；需要精准泛词或后续关键词经营时输出 `Route → 6-0-2`，并读取对应最新有效资产。

6-2 不在自身报告中重复生成关键词战略结论；字段语义、ProId、Tags `|1精准|` 和 `IsExact` 的统一定义仍由共享 Adapter/6-0-1 维护。连接失败、无匹配、ProId 缺失或语义不确定时，只记录证据缺口，不跨产品回退。
...
6-2 只识别 ASIN 经营层面的关键词异常或机会（例如 Search Term 覆盖变化、核心词自然排名信号、广告/自然词转化差异），不再复制 ERP 精准词筛选、精准泛词生成、Search Intent 聚类或 Rankability 算法。精准词问题输出 `Route → 6-0-1`；精准泛词问题输出 `Route → 6-0-2`，并读取对应最新有效资产。
