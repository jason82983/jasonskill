...
ERP 关键词是独立的 SQL 证据通道：从 `01_产品档案.md` 读取 ERP 编号，以参数化 `PickPwKView.ProId` 查询；本地 `03_系统配置` 只提供字段字典。6-2 只使用这些证据识别 ASIN 经营异常和路由，不在自身生成精准词或精准泛词；精准词路由 6-0-2，精准泛词路由 6-0-3。字段编码、金额/比例单位和排名口径未明确时继续标记 `SEMANTICS_UNCERTAIN`。
...
5. 广告整体影响：继承最新 6-3 广告诊断结论，只判断支撑增长、拖累经济性、流量不足、异常波动或过度依赖；不重复做 6-3 诊断。
...
13. 路由：广告→6-3；页面承接→5-4；文案→5-2；视觉→5-3；样品→4-1；量产→4-2；产品方案→3-2；机会失效→3-1/2-2；库存/补货→7-1/7-2；正常波动→无需处理。遵守最小必要升级。

## 反方检查

在最终状态前回答：是否把单日波动当异常；是否只看销量、ACoS 或星级；是否把销量下降直接归因广告、CVR 下降直接归因页面、竞品变化直接写成因果；是否忽略价格、Buy Box、库存、自然能力、Review重复问题；是否越权重做 6-3、5-4 或 7-1；是否可以明确标记【无需动作】。
