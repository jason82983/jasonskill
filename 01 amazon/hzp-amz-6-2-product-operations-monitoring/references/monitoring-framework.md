...
ERP 关键词是独立的 SQL 证据通道：从 `01_产品档案.md` 读取 ERP 编号，以参数化 `PickPwKView.ProId` 查询；本地 `03_系统配置` 只提供字段字典。6-2 只使用这些证据识别 ASIN 经营异常和路由，不在自身生成精准词或精准泛词；精准词路由 6-0-1，精准泛词路由 6-0-2。字段编码、金额/比例单位和排名口径未明确时继续标记 `SEMANTICS_UNCERTAIN`。
