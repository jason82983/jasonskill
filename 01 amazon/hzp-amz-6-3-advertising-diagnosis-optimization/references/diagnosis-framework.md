...
- 读取同一 Product Root 的 `01_产品档案.md`、`04_产品推广思路.md`、最新有效 6-1 新品推广报告和最新有效 6-2 产品经营监控与诊断报告。
...
6-3 先读取 6-1 的 Mother Pool、Semantic/Purchase Intent Clusters、Initial Released Keywords、Held Keywords 与成熟度。没有值得放大的成交语义方向时不扩词。1 Click/1 Order 只能是 `FIRST_ORDER_VALIDATED`；Cluster 升级 `VALIDATED` 必须结合多个相关 Search Terms/重复成交、CVR、CPA、ACoS、相关性、时间、库存和自然证据动态判断，不使用固定死门槛。
...
外部调度器可以调用 `6-3，DAILY` 或 `6-3，B2，DAILY`。每次运行顺序固定为：Identity Validation、Data Freshness Check、Advertising Performance Diagnosis、Previous Change Validation、AI Decision、Log。Daily Run 不等于 Daily Change；V1 不启用无人审批广告写入。决策状态仅允许：
...
根因优先路由：页面承接 → 5-5；经营总诊断 → 6-2；库存 → 7-1/7-2；市场 → 2-2。只有广告根因才生成广告动作。动作卡至少包含对象、Current Value、Proposed Value、Change %、Reason、Evidence、Expected Result、Risk、Validation Window 和是否需批准。允许 Hold、Bid/Budget、Bidding Strategy、Placement/Top、Exact 晋级、扩词批次、Pause/Negative、Product Target、预算迁移和局部结构调整；不为满足每日运行而强制产生动作。
...
`6-3，ALL` 对白名单和授权范围内的每个 Campaign 分别读取自己的历史、经营目标和当前广告数据，独立诊断、执行/观察和追加日志。所有 Campaign 完成后另生成《6-3 AI广告运营日报》，只汇总检查数量、修改、保持、观察、异常、Spend 变化和重大风险；日报不得塞入或替代每个 Campaign 的底层详细历史。Campaign 日志仍不进入 0-2 formal report index。
