...
### ASIN 与 SKU 的多值关系（统一模型）

`产品对应变体` 继续一行一个 SKU；解析后按 `Product_Code + Var_Code + ASIN` 归并为 `Mapped_SKUs[]`。同一 ASIN 对应多个 SKU 是合法的 `MULTI_SKU_SAME_ASIN`，不得生成重复 ASIN 报告、Campaign 或 Campaign MD，也不得取 Excel 第一行 SKU。只有具体执行接口明确要求单个 SKU 时，才从已验证的 Eligible SKU 中填充 `Primary_Advertised_SKU`，否则保持为空。

同一 `Product_Code + Var_Code` 映射到多个不同 ASIN 时，必须标记 `[VARIANT_ASIN_MAPPING_CONFLICT]` 并对写操作 Fail Closed；不得按第一行、SKU 名或库存大小猜测。`Mapped_SKUs[]`、`Advertised_SKUs[]` 固定去重、排序并以稳定格式序列化（CSV 使用 `|` 分隔）。

统一经营原则：报告以 ASIN 为主，执行与异常下钻到 SKU；ASIN 聚合前先标记 `[ASIN_AGGREGATION_SEMANTICS_CHECK]`，并记录 `Source Grain`、`Aggregation Method` 和 `Deduplication Check`。语义不明标记 `[ASIN_AGGREGATION_UNCERTAIN]`，禁止 blind SUM；库存保留 ASIN 总览和 SKU breakdown，SKU 缺货、Offer 或资格异常不得被 ASIN 总览掩盖。
...
parent_asin, child_asin, sku, mapped_skus, advertised_skus,
primary_advertised_sku, sku_count, portfolio_name, portfolio_id,
...
身份/聚合状态至少包括 `SINGLE_SKU_ASIN`、`MULTI_SKU_SAME_ASIN`、`SKU_ELIGIBILITY_PARTIAL`、`SKU_ELIGIBILITY_VERIFIED` 和 `ASIN_AGGREGATION_UNCERTAIN`。

使用以下状态记录未决问题：`[Amazon经营身份映射结构不完整]`、`[正式Product_Code缺失]`、`[店铺产品代码前缀冲突]`、`[Portfolio映射缺失]`、`[Portfolio ID验证失败]`、`[存在多个有效产品身份映射｜需要人工确认]`、`[产品身份映射异常]`、`[产品身份资料冲突]`、`[变体广告身份映射不完整]`、`[Var_Code无法唯一解析]`、`[Var_Code与Child ASIN冲突]`、`[VARIANT_ASIN_MAPPING_CONFLICT]`、`[ADVERTISED_PRODUCT_NOT_ELIGIBLE]`、`[ASIN_AGGREGATION_UNCERTAIN]`、`[Advertised Child ASIN与批准方案不一致]`、`[SellerSpace实时身份验证未完成]`。关键身份未解决时只能只读分析，禁止任何真实广告写入。
...
- 6-3 的扩词、竞品 Product Target 和其他扩展动作继承 6-1 的 Portfolio；实际 Campaign Portfolio 不匹配时允许只读诊断，但禁止写入。
- 6-2 可以按 Portfolio 聚合广告指标，但必须把 Portfolio Ad Sales 与 Product Total Sales 分开；Portfolio 身份异常路由回 6-3/6-1。
...
6-1 的审批清单和 SellerSpace Prepared Change Plan 必须做 Portfolio Name、Portfolio ID、Store、Marketplace、Own ASIN、Mapped_SKUs[]、Advertised_SKUs[] 的字段级差异比对。任一重大差异都停止执行并要求重新确认。创建成功后用同一字段结构 Read-Back，状态只能为匹配、非关键差异、无法验证或冲突；失败时保留正式报告和审计证据，不自行补救或修改旧广告。
...
## MCP Provider Boundary 与 Canonical 语义

HZP 身份与经营语义属于共享 Canonical Model，不属于 SellerSpace/优麦云专属模型。Provider Adapter 负责原始 Tool/Field/JSON/枚举/分页及指标语义映射；核心 Skill 只消费 `Product_Code`、`Var_Code`、`ASIN`、`SKU[]`、Store、Marketplace、Portfolio 和保留来源元数据的业务指标。能力状态只能是 `SUPPORTED`、`NOT_SUPPORTED`、`SEMANTICS_UNCERTAIN`；能力缺失明确标记 `[CAPABILITY_NOT_AVAILABLE]`，禁止伪造。安全写入仍需 Prepare、Exact Diff、Apply、Read-back 全链路，缺失可靠等价能力时标记 `[SAFE_WRITE_CAPABILITY_NOT_AVAILABLE]`。

当前最小纯函数边界实现位于 `01 amazon/scripts/amazon_provider_boundary.py`；它只做字段/能力/语义归一化，不包含 MCP 客户端或任何网络、广告写操作。未来 Provider 适配器应在边界层提供真实字段映射和 Capability 发现结果。

重要指标保留 `Provider`、`Source_Grain`、`Metric_Semantics`、`Attribution_Semantics`、`Data_Through`、`Freshness`、`Aggregation_Method`、`Deduplication_Status`，Provider 返回的广告归因订单与产品总订单不得静默合并。
