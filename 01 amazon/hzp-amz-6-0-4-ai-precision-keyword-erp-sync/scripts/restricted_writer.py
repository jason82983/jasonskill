"""Fail-closed, business-action-only PickPwK writer for Stage 6-0-4.
...
INPUT_NAME = "6-0-2_{product_code}_AI精准词.csv"
...
def read_input(path: str | Path) -> list[dict[str, str]]:
    ...
    if not target.exists():
        raise FileNotFoundError("[6-0-2_AI_PRECISION_KEYWORD_INPUT_MISSING]")
    with target.open("r", encoding="utf-8-sig", newline="") as handle:
        reader = csv.DictReader(handle)
        headers = tuple(reader.fieldnames or ())
        if headers == FULL_COLUMNS:
            # The canonical 6-0-2 output contains every judged row but no
