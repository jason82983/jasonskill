...
6-0-1 的 AI 判断对象是 `Current Product + Normalized Keyword`，但最终资产按真实 PickPwK Record ID 展开。Record ID 可来自当前产品或当前产品资料中明确绑定的 Benchmark；6-0-3 以该 ID 为唯一写入定位，不要求记录 `ProId` 等于本次 `Product_Code` 的当前 ERP ProId。
