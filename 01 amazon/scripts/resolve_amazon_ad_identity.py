...
def format_campaign_name(product_code: str, ad_type: str, role: str, target_match: str, sequence: int | str, var_code: str | None = None, intent_code: str | None = None, control_mode: str | None = None) -> str:
    """Format a new Campaign name using Product.Variant.AdType dot-separated identity."""
    seq = f"{int(sequence):02d}" if str(sequence).isdigit() else str(sequence)
    if control_mode is not None:
        if control_mode == "不投":
            raise ValueError("NO_INVESTMENT_HAS_NO_CAMPAIGN")
        if control_mode not in {"独立", "共享"}:
            raise ValueError("INVALID_CONTROL_MODE")
        if control_mode == "独立" and not intent_code:
            raise ValueError("INTENT_CODE_REQUIRED_FOR_INDEPENDENT_CAMPAIGN")
        if control_mode == "共享" and intent_code:
            raise ValueError("SHARED_CAMPAIGN_MUST_NOT_HAVE_INTENT_CODE")
        identity = f"{product_code}.{var_code}.{ad_type}" if var_code else f"{product_code}.{ad_type}"
        boundary = f"-{intent_code}" if intent_code else ""
        return f"{identity}-{role}-{target_match}{boundary}-{seq}"
    identity = f"{product_code}.{var_code}.{ad_type}" if var_code else f"{product_code}.{ad_type}"
    intent = f"-{intent_code}" if intent_code else ""
    return f"{identity}-{role}-{target_match}{intent}-{seq}"
...
def next_campaign_sequence(existing_names: Iterable[str], product_code: str, ad_type: str, role: str, target_match: str, var_code: str | None = None, intent_code: str | None = None, control_mode: str | None = None) -> dict[str, Any]:
    """Return an approval-time collision check; existing objects are never renamed."""
    existing = set(existing_names)
    seq = 1
    while format_campaign_name(product_code, ad_type, role, target_match, seq, var_code, intent_code, control_mode) in existing:
        seq += 1
    name = format_campaign_name(product_code, ad_type, role, target_match, seq, var_code, intent_code, control_mode)
    return {"name": name, "sequence": f"{seq:02d}", "collision": seq > 1, "reason": "Existing sequence detected" if seq > 1 else "No collision"}


_NAME_RE = re.compile(r"^(?P<product>[A-Za-z0-9]+)\.(?:(?P<variant>[A-Za-z0-9]+)-)?(?P<ad_type>SP|SB|SD)-(?:(?P<intent>[A-Za-z0-9]+)-)?(?P<role>COR|EXP|DIS|COM|CAT|DEF)-(?P<target>EXA|PHR|BRO|AUT|ASI|PT|CAT)(?:-(?P<placement>TOS|PPG))?-(?P<sequence>\d{2,})$")
_CONTROL_NAME_RE = re.compile(r"^(?P<product>[A-Za-z0-9]+)\.(?:(?P<variant>[A-Za-z0-9]+)\.)?(?P<ad_type>SP|SB|SD)-(?P<role>COR|EXP|DIS|COM|CAT|DEF)-(?P<target>EXA|PHR|BRO|AUT|ASI|PT|CAT)(?:-(?P<intent>[A-Za-z0-9]+))?(?:-(?P<placement>TOS|PPG))?-(?P<sequence>\d{2,})$")
# Older dotted-variant names remain parseable when they do not match either
# current name layout; shared names such as ``B2.M.SP-COR-EXA-01`` are now
# interpreted as control-aware shared Campaign names.
_LEGACY_NAME_RE = re.compile(r"^(?P<product>[A-Za-z0-9]+)\.(?P<variant>[A-Za-z0-9]+)\.(?P<ad_type>SP|SB|SD)-(?P<role>COR|EXP|DIS|COM|CAT|DEF)-(?P<target>EXA|PHR|BRO|AUT|ASI|PT|CAT)(?:-(?P<placement>TOS|PPG))?-(?P<sequence>\d{2,})$")


def parse_campaign_name(name: str) -> dict[str, Any]:
    """Parse current stable names; non-matching names remain legacy hints."""
    raw_name = str(name or "").strip()
    match = _NAME_RE.match(raw_name)
    control_match = _CONTROL_NAME_RE.match(raw_name) if not match else None
    if control_match:
        data = control_match.groupdict()
        return {"name": name, "status": "CURRENT_STANDARD", "product_code": data["product"], "var_code": data["variant"], "ad_type": data["ad_type"], "intent_code": data["intent"], "role": data["role"], "target_match": data["target"], "placement_experiment": data["placement"], "sequence": data["sequence"], "control_name_format": True}
    if not match:
        legacy_match = _LEGACY_NAME_RE.match(raw_name)
        if not legacy_match:
            return {"name": name, "status": "LEGACY_UNKNOWN", "product_code": None, "var_code": None, "ad_type": None, "intent_code": None, "role": None, "target_match": None, "sequence": None}
        data = legacy_match.groupdict()
        return {"name": name, "status": "LEGACY_STANDARD", "product_code": data["product"], "var_code": data["variant"], "ad_type": data["ad_type"], "intent_code": None, "role": data["role"], "target_match": data["target"], "placement_experiment": data["placement"], "sequence": data["sequence"]}
    data = match.groupdict()
    return {"name": name, "status": "CURRENT_STANDARD", "product_code": data["product"], "var_code": data["variant"], "ad_type": data["ad_type"], "intent_code": data["intent"], "role": data["role"], "target_match": data["target"], "placement_experiment": data["placement"], "sequence": data["sequence"]}
...
def classify_campaign_name(name: str, resolved_identity: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if parsed["status"] in {"CURRENT_STANDARD", "LEGACY_STANDARD"}:
        ...
        elif parsed["status"] == "LEGACY_STANDARD":
