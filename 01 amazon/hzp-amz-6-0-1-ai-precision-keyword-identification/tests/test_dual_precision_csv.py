...
def test_default_and_explicit_path_selection():
    assert module.select_paths() == ("manual", "ai")
    assert module.select_paths("手动分类精准词") == ("manual",)
    assert module.select_paths("自动分类精准词") == ("ai",)
...
def test_ai_path_and_comparison_are_independent():
    ...
    decisions = {
        ...
        "ai only": {"AI_Is_Precision": True, "AI_Confidence": "HIGH", "AI_Reason": "直接产品购买意图"},
        "both": {"AI_Is_Precision": True, "AI_Confidence": "MEDIUM", "AI_Reason": "直接相关"},
    }
    ai = module.build_ai_classified_rows(blind, decisions, product_code="B2", erp_pro_id=49727)
...
def test_ai_csv_keeps_benchmark_as_evidence_not_precision_label():
    blind = module.build_ai_blind_rows([_row("sister gift", "|1精准|")], product_code="B2", erp_pro_id=49727)
    evidence = {"sister gift": {
    ...
    ai = module.build_ai_classified_rows(
        blind,
        {"sister gift": {"AI_Is_Precision": True, "AI_Confidence": "HIGH", "AI_Reason": "购买意图明确"}},
        product_code="B2",
        erp_pro_id=49727,
...
def test_ai_three_states_retain_all_keywords_and_join_manual_label_after_blind_classification():
    rows = [_row("precision", "|1精准|"), _row("not precision"), _row("needs review")]
    blind = module.build_ai_blind_rows(rows, product_code="B2", erp_pro_id=49727)
    decisions = {
        "precision": {"AI_Classification": "PRECISION", "AI_Confidence": "HIGH", "AI_Reason": "直接购买意图"},
        "not precision": {"AI_Classification": "NOT_PRECISION", "AI_Confidence": "HIGH", "AI_Reason": "产品不匹配"},
        "needs review": {"AI_Classification": "REVIEW_REQUIRED", "AI_Confidence": "LOW", "AI_Reason": "证据不足"},
    }
    result = module.build_ai_classified_rows(blind, decisions, product_code="B2", erp_pro_id=49727, source_rows=rows)
    ...
    assert by_keyword["precision"]["Manual_Precision_Label"] == "PRECISION"
    assert by_keyword["precision"]["Comparison_Status"] == "BOTH_PRECISION"
    assert by_keyword["not precision"]["Comparison_Status"] == "BOTH_NOT_PRECISION"
    assert by_keyword["needs review"]["Comparison_Status"] == "REVIEW_REQUIRED"
    assert by_keyword["precision"]["SearchVolume30"] == 100
    assert by_keyword["precision"]["SearchVolumeDaily"] == 4
    assert by_keyword["precision"]["Tags"] == "|1精准|"
...
def test_complete_query_is_called_once_and_csv_has_bom(tmp_path):
    ...
    assert len(calls) == 1
    paths = module.write_dual_csvs(tmp_path, "B2", dual["manual_rows"], [])
    assert paths["manual"].name == "6-0-1_B2_手动分类精准词.csv"
    assert paths["ai"].name == "6-0-1_B2_AI精准词.csv"
    assert paths["manual"].read_bytes().startswith(b"\xef\xbb\xbf")
...
def test_empty_result_keeps_only_final_header(tmp_path):
    ...
    header = "自动编号,关键词,搜索量,中文名称,精准理由,精准度"
    assert paths["manual"].read_text(encoding="utf-8-sig").splitlines() == [header]
...
def test_output_paths_exclude_formal_index_root():
    ...
    assert str(paths["manual"]).endswith(r"06_SKILL分析报告\6-0-1_精准关键词识别\6-0-1_B2_手动分类精准词.csv")
    assert str(paths["ai"]).endswith(r"06_SKILL分析报告\6-0-1_精准关键词识别\6-0-1_B2_AI精准词.csv")


def test_final_assets_filter_states_and_use_exactly_six_columns(tmp_path):
    rows = [
        {"Keyword": "keep term", "KeywordCn": "保留", "SearchVolume30": 123,
         "AI_Classification": "PRECISION", "AI_Reason": "直接购买意图", "AI_Precision_Score": 88},
        {"Keyword": "drop term", "KeywordCn": "删除", "SearchVolume30": 456,
         "AI_Classification": "NOT_PRECISION", "AI_Reason": "不匹配", "AI_Precision_Score": 5},
        {"Keyword": "review term", "AI_Classification": "REVIEW_REQUIRED", "AI_Reason": "证据不足"},
    ]
    paths = module.write_dual_csvs(tmp_path, "B2", rows[:1], rows)
    assert paths["ai"].read_text(encoding="utf-8-sig").splitlines() == [
        "自动编号,关键词,搜索量,中文名称,精准理由,精准度"
    ]
    assert paths["manual"].read_text(encoding="utf-8-sig").splitlines()[0] == "自动编号,关键词,搜索量,中文名称,精准理由,精准度"
    assert "drop term" not in paths["ai"].read_text(encoding="utf-8-sig")
    assert "review term" not in paths["ai"].read_text(encoding="utf-8-sig")
...
def test_normalized_keyword_uses_one_ai_decision_for_multiple_records():
    ...
    classified = module.build_ai_classified_rows(
        blind,
        {"shared term": {"AI_Classification": "PRECISION", "AI_Precision_Score": 88, "AI_Reason": "one judgment"}},
...
def test_benchmark_only_precision_keeps_missing_current_record_marker_in_final_csv():
    ...
    ai = module.build_ai_classified_rows(pool["unresolved_candidates"], {"bench term": {"AI_Classification": "PRECISION", "AI_Precision_Score": 80, "AI_Reason": "意图匹配"}}, product_code="B2", erp_pro_id=49727, benchmark_evidence=pool["benchmark_evidence"])
...
def test_source_summary_counts_benchmark_precision_without_record_id():
    pool = module.build_keyword_candidate_pool([], [_benchmark_result(49726, "bench term")], product_code="B2", erp_pro_id=49727, benchmark_erp_pro_ids=[49726])
    ai = module.build_ai_classified_rows(pool["unresolved_candidates"], {"bench term": {"AI_Classification": "PRECISION"}}, product_code="B2", erp_pro_id=49727, benchmark_evidence=pool["benchmark_evidence"])
...
def test_semantic_sister_gifts_is_precision():
...
def test_semantic_sister_birthday_gifts_is_precision():
...
def test_low_volume_high_fit_stays_precision():
    result = module.evaluate_product_search_intent_fit(_sister_profile(), {"Core_Intent": "sister gifts"}, supporting_evidence={"SearchVolume30": 20})
    assert result["AI_Classification"] == "PRECISION"


def test_high_cpc_does_not_override_semantic_fit():
    result = module.evaluate_product_search_intent_fit(_sister_profile(), {"Core_Intent": "sister gifts"}, supporting_evidence={"CPC": 9})
    assert result["AI_Classification"] == "PRECISION"


def test_high_acos_does_not_override_semantic_fit():
    result = module.evaluate_product_search_intent_fit(_sister_profile(), {"Core_Intent": "sister gifts"}, supporting_evidence={"ACoS": 2.5})
    assert result["AI_Classification"] == "PRECISION"


def test_low_cvr_does_not_override_semantic_fit():
    result = module.evaluate_product_search_intent_fit(_sister_profile(), {"Core_Intent": "sister gifts"}, supporting_evidence={"CVR": 0.01})
    assert result["AI_Classification"] == "PRECISION"
...
def test_benchmark_precision_tag_is_not_a_label():
    result = module.evaluate_product_search_intent_fit(_sister_profile(), {"Core_Intent": "sister gifts"}, supporting_evidence={"Tags": "|1精准|"})
    assert result["AI_Classification"] == "PRECISION"
...
def test_duplicate_ids_share_one_semantic_judgment_and_expand_rows():
    ...
    fit = module.evaluate_product_search_intent_fit(profile, {"Core_Intent": "sister gifts"})
    assert fit["AI_Classification"] == "PRECISION"
    assert fit["AI_Classification"] == module.evaluate_product_search_intent_fit(profile, {"Core_Intent": "sister gifts"})["AI_Classification"]
...
if __name__ == "__main__":
    ...
    print("6-0-1 dual precision CSV tests: PASS")
