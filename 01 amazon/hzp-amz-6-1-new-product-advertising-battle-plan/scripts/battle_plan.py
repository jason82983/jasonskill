"""Build traceable 6-1 battle-plan assets from AI decisions and latest 6-0-3 inputs.
...
from scripts.new_product_battle_plan_contract import (  # noqa: E402
    BATTLE_MULTI_COLUMNS, BATTLE_SINGLE_COLUMNS, CREATION_COLUMNS, INTENT_COLUMNS,
...
METHODS = {"EXACT", "PHRASE", "BROAD", "AUTO", "ASIN", "PT", "PRODUCT", "PRODUCT_TARGET", "CATEGORY", ""}
TARGET_METHODS = {"ASIN", "PT", "PRODUCT", "PRODUCT_TARGET", "CATEGORY"}
TARGET_TYPE_BY_METHOD = {
    "ASIN": "ASIN", "PT": "PRODUCT", "PRODUCT": "PRODUCT", "PRODUCT_TARGET": "PRODUCT", "CATEGORY": "CATEGORY",
}
...
def _canonical_target_type(value: Any) -> str:
    ...
    return {
        "ASIN": "ASIN", "竞品ASIN": "ASIN", "COMPETITOR_ASIN": "ASIN",
        "PRODUCT": "PRODUCT", "PT": "PRODUCT", "PRODUCT_TARGET": "PRODUCT", "产品ASIN": "PRODUCT",
        "CATEGORY": "CATEGORY", "类目": "CATEGORY", "CATEGORY_TARGET": "CATEGORY",
        "KEYWORD": "KEYWORD", "AUTO": "AUTO",
    }.get(normalized, normalized)


def _target_fields(decision: Mapping[str, Any], method: str, keyword: str) -> tuple[str, str]:
    """Return the approved target facts that 6-2 must execute."""
    target_type = _canonical_target_type(
        decision.get("目标类型") or decision.get("Target Type") or decision.get("TargetType")
    )
    target_value = _text(
        decision.get("目标值") or decision.get("Target Value") or decision.get("Target ASIN")
        or decision.get("目标ASIN") or decision.get("Category Target") or decision.get("类目目标")
    )
    if method in {"EXACT", "PHRASE", "BROAD"}:
        if target_type and target_type != "KEYWORD":
            raise ValueError("BATTLE_TARGET_TYPE_MISMATCH")
        if target_value and target_value != keyword:
            raise ValueError("BATTLE_TARGET_VALUE_MISMATCH")
        return "KEYWORD", keyword
    if method == "AUTO":
        if target_type and target_type != "AUTO":
            raise ValueError("BATTLE_TARGET_TYPE_MISMATCH")
        return "AUTO", target_value
    if method in TARGET_METHODS:
        expected = TARGET_TYPE_BY_METHOD[method]
        if target_type and target_type != expected:
            raise ValueError("BATTLE_TARGET_TYPE_MISMATCH")
        if not target_value:
            raise ValueError("BATTLE_TARGET_VALUE_MISSING")
        return expected, target_value
    return "", ""
...
def build_tables(bundle: Mapping[str, Any], decisions: Mapping[str, Any], *, product_code: str,
    ...
    for row in keyword_rows:
        if row["计划阶段"] != "PHASE_1" or row["新品期是否投放"] != "是":
            continue
        method = row["计划投放方式"]
        target_type, target_value = _target_fields(decision_by_id[row["Id"]], method, row["词"])
        ...
        battle = {"作战单元ID": _stable_battle_id(product_code, variant_code, row["意图代码"], row["Id"], method, row["计划阶段"]),
                  ...
                  "计划阶段": row["计划阶段"], "投放方式": method, "目标类型": target_type,
                  "目标值": target_value, "作战目的": objective, "确认状态": APPROVAL}
        if multi:
...
def _ad_role(task: str, method: str) -> tuple[str, str, str]:
    method_map = {"EXACT": "EXA", "PHRASE": "PHR", "BROAD": "BRO", "AUTO": "AUT", "ASIN": "ASI",
...
def architecture_preview(tables: Mapping[str, Any], product_code: str, variant_code: str) -> dict[str, list[dict[str, Any]]]:
    ...
    for (control, ad_type, role, tool, phase, boundary), units in sorted(groups.items()):
        # Sequence 01 is a preview only; 6-2 must resolve actual collisions before apply.
...
def creation_blueprint(tables: Mapping[str, Any], architecture: Mapping[str, Any],
                       decisions: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Materialize every 6-2 creation parameter without filling missing facts by guesswork."""
    by_id = {str(row["Id"]): row for row in tables["battle"]}
    decision_by_id = {str(row.get("Id")): row for row in decisions.get("keywords", [])}
    campaign_by_id = {}
    for campaign in architecture["campaigns"]:
        for source_id in campaign["source_ids"]:
            campaign_by_id[str(source_id)] = campaign
    ad_group_by_campaign = {group["Campaign Name"]: group["Ad Group Name"] for group in architecture["ad_groups"]}
    rows = []
    for source_id in sorted(by_id, key=lambda value: int(value) if value.isdigit() else value):
        battle = by_id[source_id]
        campaign = campaign_by_id[source_id]
        decision = decision_by_id.get(source_id, {})
        params = {}
        params.update(decisions.get("campaign_parameters", {}).get(campaign["campaign_key"], {})
                      if isinstance(decisions.get("campaign_parameters", {}), dict) else {})
        params.update({key: value for key, value in decision.items() if key in {
            "Campaign Status", "Start Date", "End Date", "Portfolio Name", "Portfolio ID", "Logical Group",
            "Daily Budget", "Bidding Strategy", "Top of Search", "Rest of Search",
            "Product Pages", "Ad Group Default Bid", "Target Bid", "Target Status", "Initial Negative Targets",
            "Advertised Own ASIN", "Advertised SKU", "Advertised Product Status", "Ad Group Status", "Marketplace", "SellerSpace Store",
            "Parameter Evidence",
        }})
        missing = {field for field in (
            "Campaign Status", "Daily Budget", "Bidding Strategy", "Top of Search", "Rest of Search",
            "Product Pages", "Ad Group Default Bid", "Target Bid", "Target Status", "Portfolio Name", "Portfolio ID", "Advertised Own ASIN",
            "Advertised SKU", "Advertised Product Status", "Ad Group Status", "Marketplace", "SellerSpace Store",
        ) if not str(params.get(field, "")).strip()}
        values = {field: str(params.get(field, "DATA_NOT_AVAILABLE")).strip() or "DATA_NOT_AVAILABLE"
                  for field in CREATION_COLUMNS}
        values.update({"作战单元ID": battle["作战单元ID"], "Id": source_id,
                       "Campaign Name": campaign["Campaign Name"], "Campaign Key": campaign["campaign_key"],
                       "Ad Group Name": ad_group_by_campaign.get(campaign["Campaign Name"], "DATA_NOT_AVAILABLE"),
                       "Campaign Role": campaign["Technical Role"].split("-", 1)[0],
                       "Ad Type": campaign["广告类型"], "Logical Group": str(params.get("Logical Group", "DEFAULT")),
                       "Target Type": battle["目标类型"],
                       "Target Value": battle["目标值"], "Parameter Status": "EXECUTION_NOT_READY" if missing else "READY_FOR_6_1",
                       "Parameter Evidence": str(params.get("Parameter Evidence", "DATA_NOT_AVAILABLE")),
                       "Initial Negative Targets": str(params.get("Initial Negative Targets", "NONE_SPECIFIED")),
                       "确认状态": APPROVAL})
        rows.append(values)
    return rows
...
def render_html(product_code: str, variant_code: str, bundle: Mapping[str, Any], tables: Mapping[str, Any], architecture: Mapping[str, Any]) -> str:
    ...
    handoff = "人工补齐并批准广告创建参数后，6-2 只能读取本次有效且已批准资产；6-2 不重判意图、作战任务、阶段、投放方式或任何 Bid/Budget/Placement。缺少参数时必须返回 CREATION_BLUEPRINT_NOT_READY。"
    ...
    creation_view = [[r.get(column, "") for column in CREATION_COLUMNS] for r in tables.get("creation", [])]
    ...
    </style></head><body><main><header><p class="muted">HZP AMAZON · 6-1 NEW PRODUCT ADVERTISING BATTLE PLAN</p><h1>新品广告作战规划</h1><p>{cell(product_code)}{'.'+cell(variant_code) if variant_code else ''} · <span class="badge">PROPOSED｜待人工确认</span></p><p class="muted">6-1负责输出完整广告创建参数；6-2只做身份、状态、权限和差异校验并执行。</p></header>
    ...
    <section><h2>9. 风险与边界</h2><div class="risk"><ul><li>意图机会比是内部机会信号，不是 Amazon 官方竞争指标。</li><li>父子 Intent 汇总搜索量存在包含关系，不能跨层相加。</li><li>高度精准不等于新品期必须立即投放；预算有限时避免过度分散。</li><li>多对标排名是 Reality Evidence，不代表真实销量或市场份额；市场容量与竞争数按唯一关键词事实只计一次。</li><li>6-1必须提供创建参数；任何缺失都标记为 DATA_NOT_AVAILABLE，并阻断6-2直接执行。</li></ul></div></section>
    <section><h2>10. 广告控制权设计 · Campaign Control Design</h2><p>独立=Budget、Bid、Placement、流量隔离与增长节奏独立；共享=在兼容广告类型、角色、Target/Traffic Type、阶段及预算/广告位策略的池内低成本验证；不投=不进入当前结构。</p>{table(["Intent","作战任务","控制方式","控制原因","汇总搜索量","意图机会比"], [[r["精准泛词"],r["作战任务"],r["控制方式"],r["控制原因"],r["汇总搜索量"],r["意图机会比"]] for r in intents])}<p class="risk">6-1输出实际预算、竞价、策略和广告位参数；6-2只做运行时身份/状态/权限校验。参数不兼容或缺失时回到6-1修订。</p></section>
    <section><h2>11. 拟创建广告架构 · Proposed Advertising Architecture</h2><p>架构由 A/C/B 表派生；Sequence 01 仅为命名预览，6-2 必须按账户现存活动校验碰撞。Campaign 根据控制方式、AdType、Technical Role、Target/Traffic Type与阶段分组；独立名称含Intent Code，共享名称不含Intent Code，不投不生成Campaign。独立Intent内可包含多个Keyword。共享池的预算/Placement兼容性由执行前核验。</p>{table(["Campaign Name","Intent","意图代码","作战任务","阶段","控制方式","Technical Role","投放方式","广告类型","Target数量","计划状态","作战目的"], campaign_view)}<h3>Ad Group</h3>{table(["Ad Group Name","Target Type","Keyword Count","Match Type","Intent","作战目的"], adgroup_view)}<h3>PHASE_1 关键词追溯</h3>{table(["Id","Keyword","中文","市场容量","竞争产品数","供需比","Primary Intent","任务","控制方式","阶段","投放方式","目标类型","目标值","作战目的"], [[r["Id"],r["词"],r["中文"],r["市场容量"],r["竞争产品数"],r["供需比"],r["精准泛词"],r["作战任务"],r["控制方式"],r["计划阶段"],r["计划投放方式"],next((b["目标类型"] for b in tables["battle"] if b["Id"]==r["Id"]),"") ,next((b["目标值"] for b in tables["battle"] if b["Id"]==r["Id"]),"") ,next((b["作战目的"] for b in tables["battle"] if b["Id"]==r["Id"]),"")] for r in keywords if r["计划阶段"]=="PHASE_1" and r["新品期是否投放"]=="是"])}</section>
    <section><h2>12. 广告创建参数 · Campaign Creation Blueprint</h2><p>一行对应一个实际目标，完整列出 Campaign、Ad Group、预算、竞价、广告位、ASIN/SKU、Portfolio、否定项与运行状态。缺失值明确标为 DATA_NOT_AVAILABLE，状态为 EXECUTION_NOT_READY；6-2不得推断或补填。</p>{table(list(CREATION_COLUMNS), creation_view)}</section>
    <section><h2>13. 6-2 执行交接摘要</h2><p>{cell(handoff)}</p><p>当前状态：A/C/B/创建参数表全部 PROPOSED。只有创建参数表所有目标均为 READY_FOR_6_1，且人工作出明确批准、资产血缘、覆盖和身份验证通过后，才可进入 6-2。</p></section>
    ...
    <footer>本报告属于作战规划建议，所有行状态初始为 PROPOSED。真实投放配置、预算与竞价由 6-2 基于执行时证据再校验。</footer></main></body></html>'''
...
def write_outputs(product_root: str | Path, product_code: str, variant_code: str,
    ...
    tables["creation"] = creation_blueprint(tables, architecture, decisions)
    for key in ("intent", "keyword", "battle", "creation"):
    ...
    for key in ("intent", "keyword", "battle", "creation"):
        with paths[key].open("r", encoding="utf-8-sig", newline="") as handle:
            ...
            if tuple(reader.fieldnames or ()) != tuple(schemas[key]):
                raise ValueError("6-1_OUTPUT_READBACK_SCHEMA_MISMATCH")
            rows = list(reader)
        expected = tables[key]
        if len(rows) != len(expected) or any(
            ...
            raise ValueError("6-1_OUTPUT_READBACK_MISMATCH")
