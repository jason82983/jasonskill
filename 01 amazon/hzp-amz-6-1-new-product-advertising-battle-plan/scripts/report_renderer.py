...
*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font:15px/1.58 "Segoe UI","Microsoft YaHei",sans-serif}}header{{background:linear-gradient(120deg,#132d52,#246178);color:white;padding:34px max(22px,calc((100vw - 1320px)/2)) 30px}}h1{{margin:0 0 8px;font-size:30px}}header p{{margin:4px 0;color:#d5e3f1}}main{{max-width:1320px;margin:auto;padding:22px}}nav{{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 20px}}nav a{{color:var(--blue);background:white;border:1px solid var(--line);padding:5px 9px;border-radius:7px;text-decoration:none;font-size:13px}}section{{background:white;border:1px solid var(--line);border-radius:12px;padding:22px;margin:16px 0;box-shadow:0 3px 14px #1f335010}}h2{{font-size:21px;margin:0 0 12px}}h3{{font-size:16px;margin:18px 0 7px}}.metrics{{display:grid;grid-template-columns:repeat(auto-fit,minmax(155px,1fr));gap:10px;margin:16px 0}}.metric{{background:#f0f5fa;border-radius:9px;padding:13px 15px}}.metric strong{{display:block;color:var(--blue);font-size:24px}}.metric span{{color:var(--muted);font-size:13px}}.table-wrap{{overflow:auto;border:1px solid var(--line);border-radius:8px}}table{{border-collapse:collapse;width:100%;font-size:13px;min-width:750px}}th,td{{text-align:left;padding:9px 10px;border-bottom:1px solid var(--line);vertical-align:top;max-width:420px}}th{{background:#edf2f8;color:#30415d;position:sticky;top:0}}tr:last-child td{{border-bottom:0}}.empty,.muted{{color:var(--muted)}}.module-body{{max-height:300px;overflow:auto;padding-top:8px}}details.module{{background:white;border:1px solid var(--line);border-radius:12px;padding:0;margin:16px 0;box-shadow:0 3px 14px #1f335010}}details.module summary{{cursor:pointer;list-style:none;padding:22px;font-size:21px;font-weight:700}}details.module summary::-webkit-details-marker{{display:none}}details.module[open] summary{{border-bottom:1px solid var(--line)}}.module-guide{{margin:14px 22px;padding:10px 14px;background:#edf8f8;border-left:4px solid var(--teal);color:#29445f}}.callout{{border-left:4px solid var(--teal);background:#edf8f8;padding:12px 15px;border-radius:5px;margin:12px 0}}.warning{{border-left-color:var(--gold);background:#fff8e9}}.risk{{border-left-color:var(--red);background:#fff1ef}}.meta{{font:13px/1.7 Consolas,monospace;color:#40516b;overflow-wrap:anywhere}}.pill{{display:inline-block;padding:2px 8px;border-radius:99px;background:#e7eef8;color:#284b7d;font-size:12px}}footer{{max-width:1320px;margin:auto;padding:4px 22px 30px;color:var(--muted);font-size:12px}}
...
<script>document.querySelectorAll('main > section').forEach(function(s){{var h=s.querySelector('h2');if(!h)return;var b=document.createElement('div');b.className='module-body';while(h.nextSibling)b.appendChild(h.nextSibling);var d=document.createElement('details');d.className='module';var m=document.createElement('summary');m.textContent=h.textContent;d.appendChild(m);var g=document.createElement('p');g.className='module-guide';g.textContent='意义：展示本模块信息。作用：支持广告作战复核。为什么这样做：让结论与证据可追溯。';d.appendChild(g);d.appendChild(b);s.replaceWith(d)}});</script>
