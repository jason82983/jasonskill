# 6-2 BUILD / RECONCILE Contract

## Authority and exact input

6-2 is the apply executor, not a second planning model. Resolve the latest valid approved 6-1 bundle using `resolve_latest_approved_battle_plan`. It contains A `新品意图市场作战表.csv`, B `新品关键词作战明细.csv`, C `新品关键词阶段规划表.csv`, D `广告创建参数表.csv`, and formal HTML, all for the same Product and `RUN_ID`. The resolver checks bundle lineage, schema, approval, and target coverage. Consume only APPROVED Battle Units in the currently allowed execution phase (`PHASE_1`) with control mode `独立` or `共享`. No-invest, HOLD, proposal, later-phase or unapproved rows produce no Target. Do not bypass missing approval.

## Business-to-technical translation

Map only the approved pair. Do not alter the business task or add targets:

| 6-1 task / method | Technical Role / Target Type |
|---|---|
| 首攻/核心 + EXACT | COR / EXA |
| 扩展 + PHRASE | EXP / PHR |
| 探索 + BROAD | DIS / BRO |
...
No unique translation is `ROLE_TRANSLATION_AMBIGUOUS`. 6-1 B carries `目标类型` and `目标值`; ASIN, PT/Product Target and Category rows must provide the exact approved target facts. Fail `TECHNICAL_EXECUTION_CONFLICT` when they are missing or inconsistent; never use a Benchmark ASIN, Own ASIN or keyword text as a substitute target. Current 6-2 execution remains SP-only unless the exact create/update workflow has been validated for another AdType. The naming parser accepts SP/SB/SD, which does not authorize SB/SD execution.

## Desired Advertising State

`scripts/advertising_state_reconciler.py::build_desired_state_from_6_1` is pure: it accepts a resolved 6-1 bundle (including D), verified current Product/Variant identity and current Actual entities. It returns serializable Campaign, Ad Group, Advertised Product and Target rows. It performs no SellerSpace/Amazon I/O and does not invent Bid, Budget, Placement, Bidding Strategy, target values or statuses; D is the sole source for those fields.

Required entity fields come from D and must be copied without inference:
...
| Target | Logical ID, parent Ad Group Logical ID, 6-1 Battle Unit ID, Target Value/Type, Match Type, Bid and Status |

Execution settings must be evidence-backed and cover every approved Battle Unit. Campaign-level budget, placement and strategy, and Ad Group-level default bid/status must agree across every unit assigned to one physical Campaign/Ad Group. Any incompatible shared assignment is `SHARED_GROUP_INCOMPATIBLE`; 6-2 returns it upstream instead of silently regrouping. Target-level bids may vary.

If D has `DATA_NOT_AVAILABLE`, `待人工确认`, or `Parameter Status != READY_FOR_6_1`, return `CREATION_BLUEPRINT_NOT_READY`/`EXECUTION_NOT_READY` before provider writes. 6-2 must not substitute an economic-model bid, default budget, placement, strategy, Own ASIN/SKU, Portfolio or status.
...
Runtime Campaign Scope is mandatory and is configured in the current Product Root's `01_产品档案.md`: `ProductCode=B2`, `CampaignTag=M`, `CampaignPrefix=B2.M.`. This file is the only technical Scope source. `04_产品推广思路.md` remains the human decision reference for promotion ideas, goals, budget/inventory constraints and AI validation items; same-named fields there are not parsed for Scope. Rebuild and validate the prefix with `build_campaign_scope`; missing or conflicting values fail closed. The prefix is the only scope boundary for 6-2: retain only Campaign names that strictly `STARTS_WITH CampaignPrefix`; exclude all other ads from counts, matching, BUILD/RECONCILE, Desired State and writes. No in-scope Campaign means `BUILD`; one or more in-scope Campaigns means `RECONCILE`, regardless of other account ads. CampaignTag is an opaque identifier, not a Variant; verified real Variant remains a separate identity field. The Campaign naming contract is `{ProductCode}.{CampaignTag}.{AdType}-{Role}-{TargetType}-[IntentCode]-{Seq}`. Independent requires exactly its 6-1 Intent Code; Shared has no Intent Code; No-invest creates no Campaign. Campaign Name and run metadata must carry the exact scope.
...
- `MIGRATION_CANDIDATE`: same 6-1 Battle Unit exists under an old Campaign boundary and a new Desired target is created elsewhere; report both IDs and preserve old target pending the approved migration flow.
...
1. **Preflight:** resolve approved 6-1; verify Product/Variant/Own ASIN/SKU/Store/Marketplace/Portfolio; check page/economics/inventory/eligibility gates, role translation, target facts, parameters, sequence reservations, Logical ID uniqueness and provider write/read-back capability.
...
6. **Idempotency:** derive key from Product Code + 6-1 RUN_ID + canonical Desired State digest (not wall clock or filename). Same 6-1 run and same Actual state must yield zero CREATE and UPDATE on re-run.
...
Keep formal 6-2 HTML and 0-2 indexing unchanged. HTML/Diff CSV carry Product identity, exact A/B/C/D input names and 6-1 RUN_ID, approved Battle Unit IDs, 6-2 RUN_ID/time, BUILD/RECONCILE, Actual query time, entity/action counts, prepare/apply/read-back outcome, provider and unresolved issues. Run logs and append-only identity manifest stay out of formal 0-2 indexing.

Do not report `FULL_SUCCESS` after core 6-1 lineage, identity, actual query, apply or read-back failure. Return at least: `61_INPUT_NOT_FOUND`, `61_INPUT_RUN_MISMATCH`, `61_INPUT_SCHEMA_INVALID`, `NO_APPROVED_BATTLE_PLAN`, `INVALID_APPROVAL_STATE`, `CONTROL_MODE_MISSING`, `INTENT_CODE_MISSING`, `BATTLE_UNIT_ID_MISSING`, `ROLE_TRANSLATION_AMBIGUOUS`, `SHARED_GROUP_INCOMPATIBLE`, `CAMPAIGN_IDENTITY_CONFLICT`, `ADGROUP_IDENTITY_CONFLICT`, `TARGET_IDENTITY_CONFLICT`, `DESIRED_STATE_INVALID`, `ACTUAL_STATE_QUERY_FAILED`, `CREATE_FAILED`, `UPDATE_FAILED`, `READBACK_FAILED`, `TECHNICAL_EXECUTION_CONFLICT`, `EXECUTION_NOT_READY`, `MIGRATION_REQUIRES_APPROVAL`. Malformed 6-1 B target facts are rejected as `6-1_TARGET_VALUE_INVALID` before Desired State construction.
