...
6. 用共享 Campaign Parser 解析当前 `Product_Code.Var_Code-AdType-Role-Target/Match-Sequence` 名称中的 Product_Code、Second_Code/Var_Code 和结构；授权只做解析字段的 Exact Match，禁止 startswith/contains/substring。历史点分隔变体名称仅作兼容提示。
...
for campaign in active_campaigns:
    parsed = parse_campaign_name(campaign.name)  # Product_Code, Var_Code/Second_Code, Structure
...
ASIN-first 经营分析、同 ASIN 多 SKU、SKU Drill-down 和 Advertised Product 归因继续使用 Provider 实际数据；它们不增加自动授权复杂度。开自动的产品有.txt 不参与 6-4 最高授权判断，只保留给其它流程的兼容读取；不与 Excel 形成双重授权。
