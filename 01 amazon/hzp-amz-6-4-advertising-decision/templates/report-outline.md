...
`6-4_[产品编号]_广告诊断优化_V[版本号]_[YYYYMMDD]_[HHMMSS].html`
...
首页优先显示产品代码+中文名称、诊断窗口、6-2 来源版本、当前广告状态、最大问题层级、最大赚钱点、最大烧钱点、最大流量机会、数据不足、最优先动作、是否建议大改和是否建议保持不动。
...
产品身份、Campaign 诊断、优化动作、Expansion Proposal 和 6-5 交接必须显示 `Portfolio Name`、`Portfolio ID`、验证状态、来源和读取时间。Portfolio 与映射不一致时的动作只能是只读诊断、人工核查或路由，不得写入。
...
`ALL` 只表示所有符合精确授权和既有安全边界的广告都经过判断，不表示全部广告都会被修改。原 `开自动的产品有.txt` 只保留其它流程兼容用途，不得作为 6-4 最高授权源或扩大 Variant 权限。报告必须区分授权状态与广告表现问题。
...
同一 Campaign 的后续 Validation 必须回写同一 Campaign 的 MD 判断日志，并通过 Run_ID 回写同名 CSV 快照。Campaign 详细日志与《6-4 AI广告运营日报》分离；日志目录不进入 0-2 正式报告索引。Campaign Name 变化时必须通过 Campaign ID 找回原历史，不得错误创建新对象历史。
