...
ERP 关键词是独立的 SQL 证据通道：从 `01_产品档案.md` 读取 ERP 编号，以参数化 `PickPwKView.ProId` 查询；本地 `03_系统配置` 只提供字段字典。6-6 只使用这些证据识别 ASIN 经营异常和路由，不在自身生成精准词或精准泛词；精准词路由 6-0-2，精准泛词路由 6-0-3。字段编码、金额/比例单位和排名口径未明确时继续标记 `SEMANTICS_UNCERTAIN`。
...
5. 广告整体影响：继承最新 6-4 广告诊断结论，只判断支撑增长、拖累经济性、流量不足、异常波动或过度依赖；不重复做 6-4 诊断。
...
13. 路由：广告→6-4；页面承接→5-4；文案→5-2；视觉→5-3；样品→4-1；量产→4-2；产品方案→3-2；机会失效→3-1/2-2；库存/补货→7-1/7-2；正常波动→无需处理。遵守最小必要升级。

## 反方检查

在最终状态前回答：是否把单日波动当异常；是否只看销量、ACoS 或星级；是否把销量下降直接归因广告、CVR 下降直接归因页面、竞品变化直接写成因果；是否忽略价格、Buy Box、库存、自然能力、Review重复问题；是否越权重做 6-4、5-4 或 7-1；是否可以明确标记【无需动作】。

## 最终状态

### ASIN-first 与 SKU 下钻

同一 `Product_Code + Var_Code + ASIN` 的多个 SKU 只生成一个 ASIN 经营视图。每个指标先判断来源粒度与归因语义：独立 SKU 行可去重后聚合，已是 ASIN 汇总不得再次求和，无法确认时标记 `ASIN_AGGREGATION_UNCERTAIN`。库存同时显示 ASIN 总览和 SKU breakdown；缺货、Offer、Listing、履约或广告资格等 SKU 异常必须保留并下钻。
...
6-6 读取 6-2 正式报告、按需读取 6-4 广告报告和阶段6运行日志，但运行日志只作为【运行证据】或【历史验证】，不替代最新有效正式报告，也不进入 0-2 正式索引或最新版选择。
...
监控表增加 Portfolio Name、Portfolio ID、验证状态和来源。Portfolio 层指标与 Product 层指标分开：Portfolio Ad Sales/Orders 只表示组合聚合，Product Total Sales/Orders 只表示当前 Own ASIN、Mapped_SKUs[]/Advertised_SKUs[]。发现 Campaign Portfolio 不匹配、ID无法解析、同名多 ID 或跨店铺/站点命中时，标记 `Portfolio Identity Anomaly`，只读记录并路由，不把异常数据静默归入产品。

监控上下文沿 `Product_NewCode → Product_Code → Portfolio → Var_Code → Campaign` 继承；没有可靠 Child ASIN、Mapped_SKUs[]/Advertised_SKUs[] 关系时不做变体归因。
