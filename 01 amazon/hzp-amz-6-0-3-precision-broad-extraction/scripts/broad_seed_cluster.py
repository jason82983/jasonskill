...
INPUT_REPORT_IDENTITY = "去对标去重 高度精准词"
INPUT_REPORT_KEY = "UNIQUE_HIGH_PRECISION_KEYWORDS"
HIGH_PRECISION_NAME = "6-0-2_高度精准词表.csv"
DEDUPLICATED_NAME = "6-0-2_去对标去重 高度精准词.csv"
...
def input_paths(
    ...
    directory = Path(product_root) / "06_SKILL分析报告" / INPUT_RUN_DIR
...
def _validate_602_run_package(
    ...
    manifest_path = manifest_path or folder / f"{INPUT_602_MANIFEST_PREFIX}{stamp}.json"
    ...
    if not manifest.get("Output Folder") or Path(str(manifest["Output Folder"])).resolve() != folder.resolve():
    ...
    expected_high = [row for row in ai_rows if row.get("精准度") == "高度精准"]
    if high_rows != expected_high:
        return {"valid": False, "reason": "HIGH_PRECISION_DERIVATION_MISMATCH"}
    try:
        _validate_high_precision_rows(unique_rows)
    except ValueError as exc:
    ...
    for code, asset_rows in benchmark_rows.items():
        expected_rows = [row for row in high_rows if str(row.get("所属产品编号") or "").strip() == code]
        ...
        if (asset_rows != expected_rows
                or any(str(row.get("所属产品编号") or "").strip() != code
                       ...
                       or row.get("精准度") != "高度精准" for row in asset_rows)
                or len(canonical) != len(set(canonical))):
            return {"valid": False, "reason": "BENCHMARK_HIGH_PRECISION_DERIVATION_MISMATCH"}
    if sum(len(rows) for rows in benchmark_rows.values()) != len(high_rows):
...
def resolve_latest_valid_602_run_package(product_root: str | Path, product_code: str) -> dict[str, Any]:
    """Resolve one complete VALID 6-0-2 package, newest timestamp first."""
    directory = Path(product_root) / "06_SKILL分析报告" / INPUT_RUN_DIR
    ...
    for stamp, folder, manifest in candidates:
        package = _validate_602_run_package(folder, product_code, stamp, manifest)
        if package.get("valid"):
            return {"status": "LATEST_VALID_602_RUN_PACKAGE_READY", **package, "invalid_runs": invalid_runs}
        invalid_runs.append({"run_folder": str(folder), "run_timestamp": stamp, "reason": package.get("reason")})
    if invalid_runs:
        latest_reason = str(invalid_runs[0]["reason"]).split(":", 1)[0]
        if latest_reason in {DUPLICATE_CANONICAL_KEYWORDS, NON_HIGH_PRECISION_RECORD}:
            return {"status": latest_reason, "invalid_runs": invalid_runs}
    return {"status": MISSING_INPUT, "invalid_runs": invalid_runs}
...
def _validate_603_output_package(
    ...
    if not RUN_TIMESTAMP_RE.fullmatch(stamp):
        return {"valid": False, "reason": "RUN_TIMESTAMP_INVALID"}
    manifest_path = manifest_path or run_manifest_path(folder, stamp, legacy=legacy_folder)
    if not manifest_path.is_file():
        return {"valid": False, "reason": "RUN_MANIFEST_MISSING"}
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    ...
    if not declared_folder or Path(declared_folder).resolve() != folder.resolve():
        return {"valid": False, "reason": "RUN_FOLDER_DECLARATION_MISMATCH"}
    paths = {
    ...
    try:
        ...
        _validate_high_precision_rows(input_rows)
        input_package = _validate_602_run_package(input_folder, product_code, str(manifest["Input RUN_TIMESTAMP"]))
        if (not input_package.get("valid")
                or input_package.get("files", {}).get("deduplicated") != str(input_file)
                or input_package.get("run_id") != manifest.get("Input Run ID")
                or input_package.get("run_timestamp") != manifest.get("Input RUN_TIMESTAMP")
                or manifest.get("Input Report") != INPUT_REPORT_IDENTITY
                or manifest.get("Input Report Identity") != INPUT_REPORT_IDENTITY):
...
def resolve_latest_valid_603_run_package(product_root: str | Path, product_code: str) -> dict[str, Any]:
    ...
    candidates: list[tuple[str, Path, Path]] = []
    for manifest in directory.glob(f"{RUN_MANIFEST_PREFIX}*.json"):
...
def _validate_high_precision_rows(rows: list[dict[str, str]]) -> None:
    ...
    non_high = [row.get("Id", "") for row in rows if row.get("精准度", "") != "高度精准"]
    if non_high:
        raise ValueError(f"{NON_HIGH_PRECISION_RECORD}: {','.join(non_high)}")
...
def load_6_0_2_inputs(product_root: str | Path, product_code: str) -> list[dict[str, str]]:
    """Load UNIQUE_HIGH_PRECISION_KEYWORDS from the latest complete VALID 6-0-2 package."""
...
def build_mapping_rows(
    ...
    for row in rows:
        broad, chinese, parent = _mapping_result(map_fn(row), row)
...
def run(
    ...
    run_folder = create_run_folder(product_root, run_timestamp)
    paths = output_paths(product_root, product_code, generated_at=run_timestamp)
    ...
    manifest: dict[str, Any] = {
        ...
        "Input Source": f"6-0-2 LATEST VALID RUN PACKAGE / {INPUT_REPORT_IDENTITY}",
    ...
    try:
        ...
        mapping = build_mapping_rows(rows, mapper=mapper)
